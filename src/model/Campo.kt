...
data class Campo(val linha: Int, val coluna: Int) {
    ...
    private val vizinhacaSegura: Boolean
        get() = vizinhos.map { vizinho -> vizinho.seguro }.reduce { resultado, seguro -> resultado && seguro }
    ...
    fun abrir () {
        if (fechado) {
            aberto = true
            if (minado) callbacks.forEach { callback -> callback(this, CampoEvento.EXPLOSAO) }
            else {
