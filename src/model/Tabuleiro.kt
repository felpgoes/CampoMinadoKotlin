...
class Tabuleiro (val qtdeLinhas: Int, val qtdeColunas: Int, private val qtdeMinas: Int) {
    ...
    private fun associarVizinhos(campo: Campo) {
        ...
        val linhas = arrayOf(linha - 1, linha + 1)
        val colunas = arrayOf(coluna - 1, coluna + 1)
        linhas.forEach { l ->
            colunas.forEach { c ->
    ...
    private fun verificarDerrotaOuVitoria(campo: Campo, evento: CampoEvento) {
        if (evento == CampoEvento.EXPLOSAO) callbacks.forEach { it(TabuleiroEvento.DERROTA) }
        else if (objetivoAlcancado()) callbacks.forEach { it(TabuleiroEvento.VITORIA) }
    }
